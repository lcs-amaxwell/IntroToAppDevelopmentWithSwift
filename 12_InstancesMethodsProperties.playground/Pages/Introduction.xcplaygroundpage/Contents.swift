...
- For the type “Car” there are countless individual variations on the road, each with its own mileage history, make, model, colour, fuel level, scratches and dents. Each car also has a particular set of behaviours: its own methods for starting, braking, shifting and the like.
...
In programming, you can create and use different instances of a given type. Each instance has its own set of property values, and each instance can perform behaviour independent of other instances.
