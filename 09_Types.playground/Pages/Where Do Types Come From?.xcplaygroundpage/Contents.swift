/*:
 ...
 Notice that all the type names begin with a capital letter and — if there are multiple words — the first letter of each word is also capitalized. This is slightly different from the rules for naming constants, variables and functions, which all begin with  lowercase letters.
 
 The difference is intentional. Becuase names of types begin with a capital letter, it’s easy to tell them apart from the names of functions, variables and constants. Both use camel case, but types use _upper camel case_ as opposed to _lower camel case_.
